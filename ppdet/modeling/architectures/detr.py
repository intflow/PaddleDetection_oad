...
class DETR_oad(BaseArch):
    ...
    def __init__(self,
                 backbone,
                 transformer='DETRTransformer_oad',
                 detr_head='DETRHead_oad',
                 neck=None,
                 post_process='DETRPostProcess_oad',
                 with_mask=False,
                 exclude_post_process=False):
        super(DETR_oad, self).__init__()
        self.backbone = backbone
        self.transformer = transformer
        self.detr_head = detr_head
        self.neck = neck
...
class DETR_oadkpt(BaseArch):
    ...
    def _forward(self):
        ...
        if self.training:
            ...
            output = {'bbox': bbox, 'bbox_num': bbox_num, 'keypoint': kpts}
            if self.with_mask:
                output['mask'] = mask
            return output
